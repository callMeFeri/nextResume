...
const Page = () => {
  ...
  const Submit = async (data: TUserNameEdit) => {
    ...
    try {
      ...
      console.log("Username updated successfully:", newResponse.data);
      userData.user.username = data.userName;
      const newUserProfile = JSON.stringify(userData);
      localStorage.setItem("User profile", newUserProfile);
      router.refresh();
    } catch (error) {
            ...
            {profileEditing ? (
              <div className="w-72 gap-4">
                <form onSubmit={handleSubmit(Submit)}>
                  <div className="relative h-10 w-full min-w-[200px] flex flex-row">
                    <input
                      {...register("userName")}
                      type="text"
